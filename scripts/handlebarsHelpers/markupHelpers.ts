/**
 * The standalone handlebars version is used in the markupWithStyleForCodebox helper.
 * This is due to a bug with the engine-handlebars version for assemble, that renders partials asynchronously and therefore the codebox html cant be escaped anymore.
 */
...
export function registerHandlebarsHelpersMarkup(Handlebars, engine) {
  ...
  function generateMarkupWithStyle(context: any, modifierText: string, stateModifierText: any) {
    ...
    const markupContext = context.markupContext;
    let template;
    if (markup.search(/.*\.hbs/gi) === 0) {
      const mName = markup.replace('.hbs', '');
      const partial = Handlebars.partials[mName];
      if (!partial) {
        throw new Error(`Partial Not Found: ${mName} in 'markupWithStyle' Handlebars Helper`);
      }
  ...
  Handlebars.registerHelper('markupWithStyle', function(modifierText, stateModifierText) {
    ...
    const html = generateMarkupWithStyle(this, modifierText, stateModifierText);
  ...
  Handlebars.registerHelper('alternativeMarkupWithStyle', function(modifierText, stateModifierText) {
    ...
    const markup = context.alternativeMarkup;
    const markupContext = context.alternativeMarkupContext;
  ...
  Handlebars.registerHelper('alternative2MarkupWithStyle', function(modifierText, stateModifierText) {
    ...
    const markup = context.alternative2Markup;
    const markupContext = context.alternative2MarkupContext;
  ...
  Handlebars.registerHelper('prefixWithSrcPath', function(suffix, options) {
    ...
    // Get src path by depending scss file path
    const regEx = /[^\/]*$/;
    const path = this.srcPath.replace(regEx, '');
    return path + '/' + suffix;
